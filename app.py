...
def login():
    try:
        ...
        if not pessoa or not bcrypt.check_password_hash(pessoa.senha, str(data["senha"])):
            ...
            try:
                # O token deve expirar após 24 horas
                token = jwt.encode(
                    {"id": pessoa.id},
...
def add_pessoa():
    ...
    if "nome" in content and "email" in content:
        ...
        try:
            ...
            error_msg = "Pessoa já cadastrada na base de dados"
            return {"mesage": error_msg}, 409

        except Exception as e:
            error_msg = "Não foi possível salvar"
...
def add_pokemon():
    """Adiciona um Pokemon à base de dados."""
    ...
    if "code" in content and "fk_pessoa" in content:
        ...
        try:
            ...
            error_msg = "Não foi possível salvar Pokemon"
            return {"mesage": error_msg}, 400
    else:
